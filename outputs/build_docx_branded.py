"""
MY4 Education — Ministry Proposal DOCX  (v4 — matplotlib charts embedded)
...
Charts: real PNG images generated by gen_charts.py, embedded via add_picture()
"""
import io, zipfile, os, subprocess
...
CHARTS = "/home/user/Mahmoud-Gadalla/outputs/charts"
...
def embed_chart(doc, filename, width_inches=6.5):
    """Embed a PNG chart image centered in the document."""
...
# ── Document builder ───────────────────────────────────────────────────────────
def build():
    # Regenerate charts before building
    charts_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gen_charts.py")
    if os.path.exists(charts_script):
        import subprocess
        subprocess.run(["python", charts_script], check=True)
    ...
    embed_chart(doc, "chart_unemployment.png")

    # 3 employer stat cards
    embed_chart(doc, "chart_employer.png")
    ...
    embed_chart(doc, "chart_growth.png")
    phase_cards(doc, [
    ...
    h3(doc, "توزيع الميزانية — المرحلة التجريبية")
    embed_chart(doc, "chart_budget.png")
    ruler(doc)
