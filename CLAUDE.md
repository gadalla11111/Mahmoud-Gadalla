...
exposes three read-only tools (`crawl4ai_scrape_url`, `crawl4ai_scrape_many`,
`crawl4ai_extract_schema`) and can scrape behind a login via a saved session
(`CRAWL4AI_STORAGE_STATE` env + `save_session.py`).
