# Replace toonz path segments and user-facing 'Flare' mentions conservatively
$root = Get-Location
...
Get-ChildItem -Path $root -Recurse -File -Include $includePatterns -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    # Replace user-facing "Flare" with "Flare" for docs and desktop files only
    if ($path -match '\.md$' -or $path -match '\\xdg-data\\' -or $path -match '\.desktop$' -or $path -match '\.xml$') {
        $new = $new -replace 'Flare', 'Flare'
    }
