...
1. **Test Scrape**
   ```bash
   python3 ./scripts/scrape_apify.py --query "INDUSTRY in LOCATION" --max_items 25 --no-email-filter --output .tmp/test_leads.json
   ```
...
3. **Full Scrape**
   ```bash
   python3 ./scripts/scrape_apify.py --query "INDUSTRY in LOCATION" --max_items TOTAL_COUNT --no-email-filter --output .tmp/leads.json
   ```
